...
void *find_space_and_split(struct node *head, int new_size)
{
    ...
    while (head != NULL) {
        if (head->in_use == 0 && head->size >= new_size) {

            int remaining_size = head->size - new_size;

            if (remaining_size > 0) {
                struct node *remaining_node = node_new(remaining_size, 0);

                remaining_node->next = head->next;
                head->next = remaining_node;
            }
