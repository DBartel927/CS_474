...
#define HEADER_SIZE ((int)PADDED_SIZEOF(struct block))
#define MIN_SPLIT_REMAINDER ALIGNMENT
...
void *myalloc(int size)
{
    ...
    int needed = PADDED_SIZE(size);
    struct block *curr = head;

    while (curr != NULL) {
        if (!curr->in_use && curr->size >= needed) {
            int can_split = curr->size >= needed + HEADER_SIZE + MIN_SPLIT_REMAINDER;
        if (can_split) {
            struct block *new_block = PTR_OFFSET(curr, HEADER_SIZE + needed);
            new_block->size = curr->size - needed - HEADER_SIZE;
            new_block->in_use = 0;
        ...
        return PTR_OFFSET(curr, HEADER_SIZE);
        }
        curr = curr->next;
    }
    return NULL;
