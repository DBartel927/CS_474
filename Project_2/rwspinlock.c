...
#define WORK_USEC 50000
#define WRITER_BIT (1u << 31)

_Atomic(int) lock_word = 0;
_Atomic(int) reader_count = 0;
int writer_count = 0;
...
void read_lock(void)
{
    ...
    do {
        ...
        } while ((expected & WRITER_BIT) != 0);
    } while (!atomic_compare_exchange_weak(&lock_word, &expected, expected + 1));
}
...
void write_lock()
{
    ...
    do {
        expected = 0;
    } while (!atomic_compare_exchange_weak(&lock_word, &expected, WRITER_BIT));
}
...
{
    // TODO
    atomic_fetch_and(&lock_word, ~WRITER_BIT);
}
